...
public class ProductRepository : Repository<Product>, IProductRepository
{
    ...
    public async Task<List<Product>> GetByCategoryIdAndUserAsync(int categoryId, int userId) =>
        await _context.Products.Where(p => p.CategoryId == categoryId && p.UserId == userId).ToListAsync();

    public async Task<List<Product>> SearchByNameAndUserAsync(string name, int userId) =>
        await _context.Products.Where(p => p.Name.Contains(name) && p.UserId == userId).ToListAsync();
}
